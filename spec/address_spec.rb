...
RSpec.describe Faussaire::Address do
  describe ".city" do
    ...
    it "does not include invalid city names or names ending with a space" do
      ...
      invalid_cities = %w[Mont-de-Marsan Bastille Vichy]
      ...
      expect(all_cities & invalid_cities).to be_empty,
                                             "Invalid city names were found: #{(all_cities & invalid_cities).join(", ")}"
      expect(cities_with_trailing_space).to be_empty,
                                            "City names with trailing spaces were found: #{cities_with_trailing_space.join(", ")}"
    end
  end
  ...
  describe ".dpt_number" do
    it "is represented as a two-digit string and falls between '01' and '95'" do
      dpt_numbers = Faussaire::Address.send(:data)["fr"]["faussaire"]["address"]["dpt_number_metropolis"].map do |num|
        num.to_s.rjust(2, "0")
      end
      invalid_values = dpt_numbers.select do |num_str|
        num_str !~ /\A\d{2}\z/ || num_str.to_i < 1 || num_str.to_i > 95
      end
  ...
  it "dpt_number_other is three digits and matches one of the specified overseas values" do
    ...
    invalid_values = dpt_numbers_other.reject { |num| valid_overseas_dpt_numbers.include?(num.to_i) }
  ...
  describe ".dpt_name" do
    ...
    it "has valid department names" do
      ...
      invalid_names = dpt_names.reject { |name| name =~ valid_format_regex }
