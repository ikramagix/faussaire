  ...
  describe ".no duplicate values" do
    context "in cosmos planet list" do
      ...
      it "does not contain duplicates" do
        expect(planets.uniq.count).to eq(planets.count), "Found duplicates in planets: #{planets.group_by(&:itself).select do |_k, v|
          v.size > 1
        end.keys.join(", ")}"
      end
    ...
    context "in cosmos neighborhood list" do
      ...
      it "does not contain duplicates" do
        expect(neighborhood.uniq.count).to eq(neighborhood.count), "Found duplicates in cosmos neighborhood: #{neighborhood.group_by(&:itself).select do |_k, v|
          v.size > 1
        end.keys.join(", ")}"
      end
      ...
      it "does not contain multiple consecutive spaces" do
        neighborhood.each do |neighborhood|
          expect(neighborhood).not_to match(/\s{2,}/),
                                      "Found multiple consecutive spaces in neighborhood name: #{neighborhood}"
        end
      end
    ...
    context "in comsmic exploration list" do
      ...
      it "does not contain duplicates" do
        expect(exploration.uniq.count).to eq(exploration.count), "Found duplicates in cosmos exploration: #{exploration.group_by(&:itself).select do |_k, v|
          v.size > 1
        end.keys.join(", ")}"
      end
      ...
      it "does not contain multiple consecutive spaces" do
        exploration.each do |exploration|
          expect(exploration).not_to match(/\s{2,}/),
                                     "Found multiple consecutive spaces in neighborhood name: #{exploration}"
        end
      end
    ...
    context "in cosmos event list" do
      ...
      it "does not contain duplicates" do
        expect(events.uniq.count).to eq(events.count), "Found duplicates in cosmic events: #{events.group_by(&:itself).select do |_k, v|
          v.size > 1
        end.keys.join(", ")}"
      end
